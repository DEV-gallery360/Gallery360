...
(function($){
    ...
    */
    ...
    window.loadRentalInfo = function(sec_code){
    	var url = '';
    	//전체 대관 서비스 정보 불러오기
    	if (typeof(window.custom_name) != 'undefined' && window.custom_name != '') {
    		url = g360.root_path + "/load_rental_info_" + window.custom_name + ".mon";

    	} else {
    		url = g360.root_path + "/load_rental_info_all.mon";
    	}
    	
    	url += "?rr=" + vr_key + (sec_code ? '&sec=' + sec_code : '');
    	//rr : rental_roomkey
	...
	function hideReview() {
		// 방명록 메뉴 숨김
		$('#nav_visitor').hide();
		$('#side_nav_visitor').hide();
		$('#reviews').hide();
		
		// footer 방명록 작성영역 수정
		var $footer = $('#site_footer');
		$footer.addClass('no-review');
		$footer.find('.padding_half').removeClass('padding_half');
		$('#footer_left').removeClass('col-half-section-right').addClass('ml-5').empty();
		// 오른쪽에 위치한 내용을 왼쪽으로 이동
		$('.host-name').appendTo('#footer_left');
		$('.host-info').appendTo('#footer_left');
		$('.host-tel img').attr('src', '/img/rental/footer_ico_tel_blk.png');
		$('.host-email img').attr('src', '/img/rental/footer_ico_email_blk.png');
		$('.host-sns-wrapper').appendTo('#footer_left');
		// 로고 위로 이동
		$footer.find('address').before($('.footer-logo')).removeClass('text-light mt-4');
	}
