...
ArtDetail.prototype = {
	"init" : function(id, is_vr){
		...
		//200ms 안에 로딩 안되는 경우 loader표시
		_self.loader = setTimeout(function(){
			rs.showLoadingTop(' ');
		}, 200);
	...
	"drawRentalText" : function(){
		...
		var t1_txt = (rs.info.info.rental_type != '4' ? rt.tab1 + '소개' : rt.tab1);
		var t2_txt = (rs.info.info.rental_type != '4' ? rt.tab2 + '소개' : rt.tab2 + '정보');
