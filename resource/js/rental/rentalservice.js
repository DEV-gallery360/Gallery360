...
RentalService.prototype = {
	...
	"setLogo" : function(){
		if (this.info.logo_url) {
			...
			$('#top_logo_b').attr('src', this.info.logo_url);
		} else {
			$('#top_logo_w').attr('src', '/img/rental/logo-transparent.png');
			$('#top_logo_b').attr('src', '/img/rental/logo.png');
		}
	},
	...
	"drawRentalText" : function(){
		...
		var _html = 
			...
			'	<div class="visitor-title-quotes"><div>' + first.substring(0,2) + '</div><div class="visitor-title-quotes-back"><img src="/img/rental/visitor_quotes.png"></div></div>' + first.substr(2) +	middle + '</div> ' + 
	...
	"setArtList" : function() {
    	...
    	var artistkey = {};
    	//console.log(this);
    	//console.log(this.info.artistlist);
    	$.each(this.info.artistlist, function(){
    		//artistkey[$.trim(this.name)] = this.artistkey;
	...
	"setArtistList" : function() {
		...
		$.each(this.info.artistlist, function(idx, val){
			...
			$infos.append('<div class="btn-artist-detail animate__animated btn-rect">자세히 보기</div>');
