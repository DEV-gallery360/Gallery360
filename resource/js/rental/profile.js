...
//					return false;
	...
	"edit_education" : function(){
		...
		if( $('#career_edit2 .round_table').css('display') === 'none' ) {
			return false;
		}
		...
		html += "<div id='edu_table_area'>";
		
		
		if (typeof(gPPF_Rental.education_list) != "undefined"){
			for (var i = 0 ; i < gPPF_Rental.education_list.length; i++){
				
				var edc = gPPF_Rental.education_list[i];
				
				html += "<div id='edu_"+(i+1)+"'>"
				...
				html += "				<td>";
				html += "					<div class='btn-group'>";
				html += "						<select class='g360_custom_select' id='degree_"+(i+1)+"'>";
				html += "							<option value='박사'>박사</option>";
				html += "							<option value='석사'>석사</option>";
				html += "							<option value='학사'>학사</option>";
				html += "							<option value='전문사'>전문사</option>";
				...
				html += "				<td>";
				html += "					<div class='btn-group'>";
				html += "						<select class='g360_custom_select' id='fin_edu_"+(i+1)+"'>";
				html += "							<option value='졸업'>졸업</option>";
				html += "							<option value='중퇴'>중퇴</option>";
				html += "							<option value='재학'>재학</option>";
				html += "							<option value='수료'>수료</option>";
				html += "						</select>"
				html += "					</div>";
				
				html += "					<button class='btn btn_cell_delete' onclick=\"gPPF_Rental.delete_id('edu_"+(i+1)+"')\">삭제</button>"
				...
				html += "			<tr>";
				html += "				<th>학교명</th>";
				html += "					<td><input type='text' class='txt' value='"+edc.schoolname+"' id='schoolName_"+(i+1)+"'  /></td>";
				html += "				<th>전공</th>";
				html += "					<td><input type='text' class='txt'  value='"+edc.major+"' id='major_"+(i+1)+"'  /></td>";
				html += "			</tr>";
		...
		html += "			<button class='btn btn_violet btn_cell_add' onclick=\'gPPF_Rental.add_education();\'>추가</button>";
		
		html += "			<button class='btn btn_gray btn_cancel' onclick=\"gPPF_Rental.load_career_list('education');\">취소</button>";
		html += "			<button class='btn btn_violet btn_ok' onclick=\"gPPF_Rental.save_education();\">수정 완료</button>";
		html += "	</div>";
		
		
		$("#career_edit2").append(html);
		
		$("#career_edit2 .round_table").hide();
		
		if (typeof(gPPF_Rental.education_list) != "undefined"){
			for (var i = 0 ; i < gPPF_Rental.education_list.length; i++){
				var edc = gPPF_Rental.education_list[i];
				$("#degree_"+(i+1)).val(edc.level).prop("selected",true);
				$("#fin_edu_"+(i+1)).val(edc.status).prop("selected",true);
			}
		}
	...
	"add_education" : function(){
		...
		//console.log($("#edu_table_area>div:nth-last-child(1)").attr('id').replace("edu_",""));
		
		//var i = $("#edu_table_area table").length;
		
		var i = 0;
		if ($("#edu_table_area>div:nth-last-child(1)").length == 0){
		}else{
			i = $("#edu_table_area>div:nth-last-child(1)").attr('id').replace("edu_","");
		}
		...
		html += "<div id='edu_"+(i+1)+"'>"
		html += "	<table class='t_education_edit t_block' id='edu_"+(i+1)+"'>";
		html += "		<colgroup>";
		html += "			<col class='t_th' />";
		html += "			<col />";
		html += "			<col class='t_th' />";
		...
		html += "			<tr>";
		html += "				<th>학교명</th>";
		html += "					<td><input type='text' class='txt' id='schoolName_"+(i+1)+"' /></td>";
		html += "				<th>전공</th>";
		html += "					<td><input type='text' class='txt' id='major_"+(i+1)+"'  /></td>";
		html += "			</tr>";
	...
	"save_education" : function(){
		...
		$("#edu_table_area>div").each(function(index){
			
			//- Put
			
			var i = this.id.replace("edu_","");
			
			var edu_schoolname = $("#schoolName_" + i).val();
			var edu_major = $("#major_"+i).val();
			var edu_start = $("#start_"+i).val();
			var edu_end = $("#end_"+i).val();
			
			var edu_level = $("#degree_"+i).val();
			var edu_status = $("#fin_edu_"+i).val();
			...
			if (edu_schoolname == ""){
				if(isOK){ g360.gAlert("Info","학교명을 입력하세요", "blue", "top"); }
				isOK = false;
...
//			html += "							</tr>";
	...
	//	html += "				<button class='btn btn_gray' id='artist_list' onclick=\"gPPF_Rental.save_artist_info_list();return false\">목록</button>";
		...
		//이미지 삭제
		html += "								<img src='/img/account/btn-close.svg' alt='삭제' id='artist_del_img' style='display:none'>";	
		...
		//이미지 삭제
		html += "								<img id='artist_del_img_detail' src='/img/account/btn-close.svg' >"
		html += "							</div>";
	...
	"imageEventBind" : function(){
		...
		$("#artist_del_img").on("click",function(){

			var ch = confirm("정말 삭제하시겠습니까?");
			if(!ch) return false;
			
			var data = JSON.stringify({
				email : gPPF_Rental.cUserInfo.email,
				type : "artist"
			})
			...
			$.ajax({
				...
				dataType:"json",
				contentType:"application/json; charset=utf-8",
				url: g360.root_path+"/FileDelete_artist.mon",
				success: function(data){
					var res = data.result;
					if(res=="OK"){
						alert("삭제 성공");
					...
					}else if(res=="no_exist"){
						alert("파일이 존재하지 않습니다.");
						
					}
					
					gRen.go_profile_rental(gPPF_Rental.cUserInfo.email);
				}
		...
		$("#artist_del_img_detail").on("click",function(){
...
//			  	gRen.load_art_list(0, "init");
	...
	"save_artist_info_reg" : function(){
		...
		var name = $.trim($("#artist_reg_name").val());
		var ename = $.trim($("#artist_reg_engname").val());
		var content = $("#artist_reg_content1").val();
		var content2 = $("#artist_reg_content2").val();
		
		if (name == ""){
			$("#artist_reg_name").val('').focus();
			g360.gAlert("Info","이름(활동명)은 반드시 입력하셔야 합니다.", "blue", "top");
			return false;
		}
		
		if (ename == ""){
			$("#artist_reg_engname").val('').focus();
