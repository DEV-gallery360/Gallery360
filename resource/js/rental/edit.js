...
gRental_Edit.prototype = {		
		...
		*/
		...
		$("#pop1").on("click", function(){			
		   ...
		   //   window.open(url, "communicationViewPopup", "width=750, height=700;");	
		      gRE.open_subwin(url, "750", "800", false, "info", false);
		});
		//개인정보 취급 클릭시
		$("#pop3").on("click", function(){
			var url = "/admin_new/terms.jsp"
		//	gRE.layer_popup_url(url, "1");			
			gRE.open_subwin(url, "1250", "800", false, "info", false);
		});
		//이용약관 취급 클릭시
		$("#pop2").on("click", function(){
			var url = "/admin_new/privacy.jsp"
		//	gRE.layer_popup_url(url, "2");			
			gRE.open_subwin(url, "1250", "800", false, "info", false);
		});
	...
	"save_main_select" : function() {
		...
		$.ajax({
			...
			success : function(res){
				if (res.result == "OK"){			
					gRE.gAlert("Info","정상적으로 반영되었습니다.", "blue", "top", function(){
						location.reload();						
					});
				}else{
					gRE.gAlert("Info","오류가 발생하였습니다.", "blue", "top");
	...
	"open_image_change" : function(){
		...
		$("#vrgallery_popup_title_rental").html(bar + "전시 이미지 선택");		
	...
	"edit_init" : function(){
		...
		$("#date1").val(info.start_date);
		$("#date2").val(info.end_date);
		...
		//$("#reg_sec_code").val(info.sec);
		
		//투표 기능
		$('input:radio[name="vote"]:radio[value="'+info.vote_use+'"]').prop('checked', true);
		
		//보안 코드
		$('input:radio[name="secret_code"]:radio[value="'+info.sec_use+'"]').prop('checked', true);
		
		
		if(info.vote_use=="T"){
			$(".vote_detail").css("visibility","visible");
			$('.vote_detail select').val(info.vote_count).prop("selected",true);
		}else{
		...
		if(info.sec_use=="T"){
			...
			$(".secret_code_input").val(info.sec);
		}else{
	...
	"dbook_upload_check" : function(){
		...
		if($('input:radio[name="secret_code"]:checked').val()=="T"){
			...
			if(v==""){
				gRE.gAlert_focus("Info","보안코드를 입력해주세요.", "blue", "top", "reg_sec_code");
				return false;
			} 
		}
