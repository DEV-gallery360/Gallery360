...
//		g360.loadingbar_open("인공지능 큐리가 고객님의 사용 패턴을 분석중입니다");
//		
...
//					});
		...
		if (opt == "facebook"){
			...
			$.ajax({
				...
				success : function(res){
					if (res.result == "OK"){
						g360.gAlert("Info","홈페이지에 비공개 처리 되었습니다.", "blue", "top");
					}else{
						g360.gAlert("Info","홈페이지에 공개 되었습니다.", "blue", "top");
					}
					return false;
				},
				error : function(e){
	...
	//	var email = dockey.split("_")[0];
		...
		Kakao.Link.sendDefault({
		  objectType: 'feed',
		  content: {
			title: "Gallery360 인공지능 AI Painter",
			description: '',
			imageUrl: uuu,
			link: {
			  webUrl: uu,
