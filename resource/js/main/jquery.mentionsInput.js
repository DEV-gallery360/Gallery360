...
//          autocompleteListItem       : _.template('<li data-ref-id="<%= id %>" data-ref-type="<%= type %>" data-display="<%= display %>"><%= content %></li>'),
            autocompleteListItem       : _.template('<li data-ref-id="<%= id %>" data-ref-type="<%= type %>" data-display="<%= display %>"><dl><dt><%= content %></dt><dd><ul><span class="mentions-team"><%= content_dept %></span></ul></dd></dl></li>'),
            autocompleteListItemAvatar : _.template('<img src="<%= avatar %>" onerror=\"g360.no_photo_draw(this)\" />'),
            autocompleteListItemIcon   : _.template('<div class="icon <%= icon %>"></div>'),
            mentionsOverlay            : _.template('<div class="mentions"><div></div></div>'),
            mentionsDisplay            : _.template('<ul class="mention_list">'),
            mentionItemSyntax          : _.template('@[<%= value %>](<%= type %>:<%= id %>)'),
            selectedMention            : _.template('<li id="li-mention-<%= id %>"><span><%= value %></span><button id="<%= id %>" name="del-mention" class="btn" style="position:inherit">삭제</button></li>')
        }
    };
    ...
    var MentionsInput = function (settings) {
        ...
        //	debugger;
            elmMentionsDisplay = $(settings.templates.mentionsDisplay()); //Get the HTML code of the mentions' overlay
         //  elmMentionsDisplay.prependTo(elmWrapperBox); //Insert into elmWrapperBox the mentions overlay
            elmMentionsDisplay.appendTo(elmWrapperBox); //Insert into elmWrapperBox the mentions overlay
        }
        ...
        function updateValues() {
            ...
            _.each(mentionsCollection, function (mention) {
                ...
                var textSelectedMention = settings.templates.selectedMention(formattedMention);
                mentionListHtml.push(textSelectedMention);
                mentionText = mentionText.replace(new RegExp(utils.regexpEncode(textSyntax), 'g'), textSelectedMention);
            });
	        ...
	        elmSelectedMention = $( "button[name*='del-mention']" )
	        elmSelectedMention.bind('click', removeSelectedMention);
        }
        ...
        */
        ...
        function onInputBoxKeyUp(e) {
        	...
        	if(e.keyCode !== KEY.BACKSPACE) {
                var InputBoxText = $("#talk_text").val();
                inputBuffer = Hangul.disassemble(InputBoxText);
           	}
