...
gVrArtDetail.prototype = {		
	...
	"g_lang" : function(){
		$('.g_lang_VR_Detail_Tab_1').html(g360.g_lang.VR_Detail_Tab_1);
		$('.g_lang_VR_Detail_Tab_2').html(g360.g_lang.VR_Detail_Tab_2);
	...
	"eventBind" : function(){
		...
		// 장바구니 --> 즐겨찾기 기능으로 변경(20.3.3) --> 구매하기 기능으로 명칭 변경하여 작품상세로 링크(22.2.15)
		_self.wrapper.find('.btn_curie_buy').on('click', function(){
			...
			*/
			...
			// 구매하기 기능
			var _url = '/index.jsp?ty=link_art&id=' + _self.cur_art_info.dockey;
			window.open(_url);
			return false;
		});
