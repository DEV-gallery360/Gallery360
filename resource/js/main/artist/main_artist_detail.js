...
gArtistDetail.prototype = {		
	...
	"init" : function(id, call_type){
		...
		// 구글 통계 데이터 생성
		var link_path = g360.getLinkPath('link_artist', id);
	...
	"g_lang" : function(){
		$(".g_lang_Artist_Detail_1").html(g360.g_lang.Artist_Detail_1);
		$(".g_lang_Artist_Detail_2").html(g360.g_lang.Artist_Detail_2);
		$(".g_lang_VRGallery").html(g360.g_lang.VRGallery);
		$(".g_lang_Link").html(g360.g_lang.Link);
		$(".g_lang_ArtistNote").html(g360.g_lang.ArtistNote);
	...
	showArtistInfo : function(data){
		...
		$('#artist_info_name').prepend(data.nickname);
		...
		// 공유하기
		$('#btn_link_artist').on('click', function(){
			var link_path = g360.getLinkPath('link_artist', data.email);
			g360.copyToClipboard(link_path);
		});
		
		// 공유하기 - 작품
		$('#btn_link_artist_art').on('click', function(){
			var link_path = g360.getLinkPath('link_artist_art', data.email);
			g360.copyToClipboard(link_path);
		});
		
		// 공유하기 - VR
		$('#btn_link_artist_vr').on('click', function(){
			var link_path = g360.getLinkPath('link_artist_vr', data.email);
			g360.copyToClipboard(link_path);
		});
		...
		var career = '';
		sep = '';
		if (!data.career || data.career.length == 0 || data.career[0].title.replace(/\s*/g, "") == "") {
		...
		var cert = '';
		sep = '';
		if (!data.cert || data.cert.length == 0 || data.cert[0].certname.replace(/\s*/g, "") == "") {
		...
		var display = '';
		sep = '';
		if (!data.display || data.display.length == 0 || data.display[0].title.replace(/\s*/g, "") == "") {
		...
		return $.ajax({
			...
			success : function(data){
				...
				*/
				...
				if (data[0].totalcount == 0) {
					$grid.html('<div class="no-list"><div class="center-middle">'+g360.g_lang.Nohave_purchase1+'</div></div>');
				}
